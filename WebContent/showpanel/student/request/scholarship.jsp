...
<div>
	<table width="90%" align="center">
		...
		<tr height="30px">
			...
			<th colspan="3"><form action="student/request.action"
					...
					<input name="action" value="search" hidden="true" /> <input
						name="type" value="scholarship" hidden="true" /> 关键字：<input
						type="search" name="word"> <input type="submit" value="搜索">
				</form></th>
		</tr>
		<%
			List<ScholarShip> scholarships = (List<ScholarShip>) session.getAttribute("scholarships");
			int pageNumber = 1;
			try {
				pageNumber = Integer.parseInt(request.getParameter("pageNumber"));
			} catch (Exception e) {
			}
			int allpages = scholarships != null ? (scholarships.size() / 10 + ((scholarships.size() % 10 == 0) ? 0 : 1))
					: 0;
		...
		for (int i = 0; i < 10; i++) {
			...
			if (idx >=  scholarships.size()) {
				break;
			}
			 ScholarShip ship =  scholarships.get(idx);
		%>
		<tr height="30px">
			<td width="10%"><%=i + 1%></td>
			<td width="30%"><%=ship.getName()%></td>
			<td width="30%"><%=ship.getRequest().substring(0, ship.getRequest().length() < 10
									? ship.getRequest().length() : 10) + "..."%></td>
			<td width="15%"><%=ship.getAddtime().toString().substring(0, 10)%></td>
			<td width="15%">
				<%
					if (ship.isActivition()) {
				%><a
				href="student/request.action?type=scholarship&action=add&id=<%=ship.getId()%>">申请</a>
				<%
					} else {
				%>无法申请<%
					}
