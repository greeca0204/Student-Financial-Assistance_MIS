<%@page import="com.slg.zxgl.entity.GrantShipApplication"%>
<%@page import="java.util.List"%>
<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"%>
<div>
	<%
		List<GrantShipApplication> ships = (List<GrantShipApplication>) session.getAttribute("grantships");
		int pageNumber = 1;
		try {
		...
		System.out.println(pageNumber + "::" + allpages);
		if (ships != null) {
	%>
	<table align="center" width="95%" style="margin-top: 10px">
		...
		<%
			for (int i = 0; i < 10; i++) {
					...
					GrantShipApplication shipapp = ships.get(idx);
		%>
		<tr height="25px">
			<td width="10%"><%=i + 1%></td>
 ...
 %><a href="manager/audit.action?type=grantship&action=show&id=<%=idx%>">进入审核</a>
				<%
					}
				%>
			</td>
